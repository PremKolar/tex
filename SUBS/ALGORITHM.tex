...
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \section{Step S03: Find Contours}
...
% ----------------------------------------------------------------------
\paragraph{Logical Mask of Eddy Interiour}\label{filter:cutmask}
...
\end{figure}
\paragraph{Area} \label{filter:area}
...
\ref{filter:dynscale}.
% ----------------------------------------------------------------------
\paragraph{Shape}\label{filter:shape}
...
\end{figure}
% ----------------------------------------------------------------------
\paragraph{Amplitude} \label{filter:amp}
...
% ----------------------------------------------------------------------
\paragraph{Chelton's Scales} \label{filter:chstuff}
...
\begin{enumerate}
	...
	\item
	The scale $L_s$ which is \textit{a direct estimate based on the contour of SSH within the eddy interior around which the average geostrophic speed is maximum.} \citet{Chelton2011} It is hence conceptually the same as $\sigma$ \TODO{legend ref}. This scale was not calculated here, as I could not think of an efficient simple way to estimate the area bounded by maximum geostrophic speed \ie the zero-vorticity contour. To understand why this would be difficult to achieve see also sections... \TODO{ref to approp sections eg noise in vort etc}.
\end{enumerate}
% ----------------------------------------------------------------------
\paragraph{Profiles}\label{filter:profiles}
...
\end{marginfigure}
% ----------------------------------------------------------------------
\paragraph{\textit{Dynamic} Scale ($\sigma$)} \label{filter:dynscale}
...
the eddy's peak in y- (x-) directions \footnote{The velocities are calcualted from the gradients of 4th-order Fourier fits to the SSH profile in respective direction (see \TODO{ref}).}.
...
% ----------------------------------------------------------------------
\paragraph{\textit{Dynamic} Amplitude} \label{filter:ampDyn}
...
\end{marginfigure}
% ----------------------------------------------------------------------
\paragraph{Center of Volume} \label{filter:CoV}
...
% ----------------------------------------------------------------------
\paragraph{Geo Projection}\label{filter:projLocs}
%\mcode{function EddyArea2Ellipse}\\
...
\mcode{function S04_track_eddies}\\
...
\TODO{relevant only for chelton method!}
\end{itemize}

...
\begin{marginfigure}
	...
	\caption{Same plot as the one \popSevenII~one from \cref{fig:ScheltsAll} with the result from \popTwoII added in brown.}
...
\setlength\itemsep{0cm}
	\item 
 Westward drifts are now faster in low latitudes, suggesting that the 7-day time-step is in deed too large to correctly associate all of the large, fast tropical eddies.
 \item
...
The reversal in drift direction within all sufficiently strong eastward currents (\eg ACC) shows that the, naturally westward-propagating eddies get advected by mean flows, analogous to eastward-moving pressure systems spawned off of the atmospheric jet-stream. \TODO{depth, dingsbumsdepth, bla} 
